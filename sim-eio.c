...
static long long int fastfwd_count;
...
sim_check_options(struct opt_odb_t *odb, int argc, char **argv)
{
  if (fastfwd_count < 0 || fastfwd_count >= 2147483647)
    fatal("bad fast forward count: %ld", fastfwd_count);
  if(first_timmer) {
	  fastfwd_count *= 1000000;
	  first_timmer = 0;
  }
}
  ...
  if (fastfwd_count > 0)
    {
      ...
      fastfwding = TRUE;
      for (icount=0; (icount < fastfwd_count) && !program_complete; icount++)
	{
